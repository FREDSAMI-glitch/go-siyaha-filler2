...
def add_docx_defaults(context):
    ...
    for key in ["dossier", "entreprise", "dirigeant", "projet", "banque", "emplois", "financement_pi", "financement_expert", "hypotheses_financieres", "financement_checkbox"]:
    ...
    for section in ["dossier", "entreprise", "dirigeant", "projet", "banque", "emplois", "financement_pi", "financement_expert", "hypotheses_financieres"]:
        for key, value in common_defaults.items():
    ...
    context["banque"].setdefault("capital", context["banque"].get("capital_social", context["banque"].get("capital_social_banque", "")))
    ...
    if isinstance(investissements, list):
        for item in investissements:
            if not isinstance(item, dict):
                continue
            cat = (item.get("categorie") or item.get("category") or item.get("type") or "").lower()
            if "terrain" in cat:
...
def apply_dap_default_mappings(doc, context):
    ...
    for item in default_mappings:
        ...
        if value not in ["", None, 0]:
            fill_cell_next_to_label(doc, item["label"], value, item.get("position", "right"), int(item.get("occurrence", 1)))
...
def apply_dap_mapping_file(doc, context):
    ...
    label_mappings = mapping.get("cell_mappings", []) + mapping.get("label_mappings", []) + mapping.get("field_mappings", [])
...
async def fill(request: Request):
    try:
        ...
        if selected_template == "bp_excel":
            ...
            add_docx("engagement_capacite_financiere.docx", f"{identifiant}_{societe}_engagement_capacite_financiere.docx")

        elif selected_template == "engagement_autorisations":
            add_docx("engagement_autorisations.docx", f"{identifiant}_{societe}_engagement_autorisations.docx")

        elif selected_template == "declaration_honneur_justificatifs":
            add_docx("declaration_honneur_justificatifs.docx", f"{identifiant}_{societe}_declaration_honneur_justificatifs.docx")
        ...
        if not generated_files:
            return JSONResponse(status_code=400, content={"error": "Aucun fichier généré. Vérifiez les templates."})

        with zipfile.ZipFile(output_zip, "w", zipfile.ZIP_DEFLATED) as zipf:
            for file_path in generated_files:
                zipf.write(file_path, arcname=file_path.name)

        return FileResponse(path=str(output_zip), media_type="application/zip", filename=output_zip.name)
